 ...
 * for firing events while the button is held down.  There is a default initial delay of 300ms before the first event is
 * fired and a 60ms delay between subsequent events.  When the user holds the button down and moves the mouse out from
 * over the button, the timer stops, but if the user moves the mouse back over the button without having released the
 * mouse button, the timer starts up again at the same delay rate.  If the enabled state is changed while the timer is
 * active, it will be stopped.
 * <p>
 * NOTE:  The normal button behavior is that the action event is fired after the button is released.  It may be
